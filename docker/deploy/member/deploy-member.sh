...
do
  ...
  HEALTH_CHECK_RESPONSE=$(curl -s http://127.0.0.1:${START_PORT}/${APP_NAME}/ping | grep 'UP')
...
sudo sed -i "s/${TERMINATE_PORT}/${START_PORT}/" /home/ec2-user/mashup-serverdocker/infra/nginx/conf.d/app.conf
